...
function formatDDay(d: number | null, isCompleted: boolean): React.ReactNode {
    ...
    if (d < 0) return <span className="text-red-600 text-xs font-bold">{Math.abs(d)}일 초과</span>
    if (d === 0) return <span className="text-red-600 text-xs font-bold">D-Day</span>
                        ...
                        {sorted.map((entry) => {
                            ...
                            return (
                                <tr
                                    ...
                                    className={`${alertRed ? 'bg-red-50' : alertAmber ? 'bg-amber-50' : ''} transition-colors`}
                                >
                                    <td className="py-2 pr-4">
                                        <span className={`text-sm font-semibold ${alertRed ? 'text-red-700' : alertAmber ? 'text-amber-700' : 'text-foreground'}`}>
                                        ...
                                        <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${isCompleted ? 'bg-emerald-100 text-emerald-700' :
                                                entry.status === 'PENDING_APPROVAL' ? 'bg-indigo-100 text-primary/90' :
                                                    entry.status === 'REVIEW' ? 'bg-amber-100 text-amber-700' :
                                                        entry.status === 'NOT_STARTED' || entry.status === 'DRAFT' ? 'bg-gray-100 text-gray-400' :
