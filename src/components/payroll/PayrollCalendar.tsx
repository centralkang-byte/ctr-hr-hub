...
import type { PipelineEntry } from './PayrollPipeline'
...
function formatDDay(d: number | null, isCompleted: boolean): React.ReactNode {
    if (isCompleted) return <span className="text-[#059669] text-xs">완료</span>
    if (d == null) return <span className="text-[#999] text-xs">—</span>
    if (d < 0) return <span className="text-[#DC2626] text-xs font-bold">{Math.abs(d)}일 초과</span>
    if (d === 0) return <span className="text-[#DC2626] text-xs font-bold">D-Day</span>
    return <span className={`text-xs font-semibold ${d <= 3 ? 'text-[#D97706]' : 'text-[#555]'}`}>D-{d}</span>
}
                        ...
                        {sorted.map((entry) => {
                            ...
                            return (
                                <tr
                                    ...
                                    className={`${alertRed ? 'bg-[#FFF5F5]' : alertAmber ? 'bg-[#FFFBEB]' : ''} transition-colors`}
                                >
                                    <td className="py-2 pr-4">
                                        <span className={`text-sm font-semibold ${alertRed ? 'text-[#B91C1C]' : alertAmber ? 'text-[#B45309]' : 'text-[#1A1A1A]'}`}>
