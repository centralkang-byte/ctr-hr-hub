...
const STATUS_CONFIG: Record<string, { label: string; bg: string; text: string; border: string }> = {
  DRAFT: { label: '초안', bg: 'bg-background', text: 'text-muted-foreground', border: 'border-border' },
  ATTENDANCE_CLOSED: { label: '근태 마감', bg: 'bg-primary/10', text: 'text-emerald-700', border: 'border-emerald-200' },
  CALCULATING: { label: '계산 중', bg: 'bg-amber-500/15', text: 'text-amber-700', border: 'border-amber-300' },
  ...
  PENDING_APPROVAL: { label: '결재 대기', bg: 'bg-amber-500/15', text: 'text-amber-700', border: 'border-amber-300' },
  APPROVED: { label: '승인 완료', bg: 'bg-emerald-500/15', text: 'text-emerald-700', border: 'border-emerald-200' },
  PAID: { label: '지급 완료', bg: 'bg-indigo-500/15', text: 'text-primary/90', border: 'border-indigo-200' },
  PUBLISHED: { label: '확정', bg: 'bg-indigo-500/15', text: 'text-primary/90', border: 'border-indigo-200' },
