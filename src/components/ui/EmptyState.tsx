...
export function EmptyState({ icon: Icon = Inbox, title, description, action }: EmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4">
      ...
      <h3 className="text-sm font-medium text-gray-900 mb-1">{resolvedTitle}</h3>
      {resolvedDesc && <p className="text-xs text-gray-500 text-center max-w-sm">{resolvedDesc}</p>}
      {action && (
        action.href ? (
