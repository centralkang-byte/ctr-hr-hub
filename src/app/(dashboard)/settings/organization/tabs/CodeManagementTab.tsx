  ...
  return (
    <div className="space-y-4">
      <div className="mb-4 flex items-center justify-between">
        <div><h3 className="text-base font-semibold text-foreground">{t('codeManagement')}</h3><p className="text-sm text-muted-foreground">시스템 코드/열거형 {Object.keys(grouped).length}개 그룹</p></div>
      ...
      {Object.keys(grouped).length > 0 ? Object.entries(grouped).sort(([a],[b]) => a.localeCompare(b)).map(([group, items]) => (
        <div key={group}>
          <h4 className="mb-2 text-sm font-semibold text-muted-foreground">{group} ({items.length})</h4>
        ...
        <div className="rounded-xl border border-dashed border-border py-12 text-center">
          <Hash className="mx-auto mb-3 h-8 w-8 text-muted-foreground" /><p className="text-sm font-medium text-foreground">{t('register_keb909c_kecbd94eb_kec9786ec')}</p>
        </div>
      )}
    </div>
  )
