...
export default function WorkforceClient() {
  ...
  if (loading) {
    return (
      <AnalyticsPageLayout title="인력 분석" description="부서별, 고용형태별, 직급별 인원 분포">
  ...
  if (!data) {
    return (
      <AnalyticsPageLayout title="인력 분석">
        <EmptyChart />
      </AnalyticsPageLayout>
    )
  }
  ...
  return (
    <AnalyticsPageLayout title="인력 분석" description="부서별, 고용형태별, 직급별 인원 분포">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {/* 부서별 인원 */}
        <ChartCard title="부서별 인원수" className="lg:col-span-2">
          {data.byDepartment.length > 0 ? (
            <ResponsiveContainer width="100%" height={Math.max(300, data.byDepartment.length * 40)}>
              <BarChart data={data.byDepartment} layout="vertical" margin={{ left: 100 }}>
                ...
                <Tooltip />
                <Bar dataKey="headcount" fill="#2563EB" radius={[0, 4, 4, 0]} name="인원" />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <EmptyChart />
          ...
          {data.byGrade.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={data.byGrade}>
                ...
                <YAxis />
                <Tooltip />
                <Bar dataKey="headcount" fill="#10B981" radius={[4, 4, 0, 0]} name="인원" />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <EmptyChart />
