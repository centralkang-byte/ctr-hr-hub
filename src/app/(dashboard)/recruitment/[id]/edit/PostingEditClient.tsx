...
export default function PostingEditClient({
 user, id }: Props) {
  ...
  const handleAiGenerate = async () => {
    ...
    setAiLoading(true)
    try {
      const res = await apiClient.post<{ description: string; qualifications: string; preferred: string }>('/api/v1/ai/job-description', {
        title, department: dept?.name, grade: grade?.name, category: cat?.name,
      })
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select {...register('workMode')} className={inputClass}>
                {WORK_MODE_OPTIONS.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
              </select>
            </div>
            <div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 mb-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea {...register('description')} rows={5} className={inputClass} placeholder={t('descriptionPlaceholder')} />
              {errors.description && <p className={errorClass}>{errors.description.message}</p>}
            </div>
            <div>
              <label className={labelClass}>{t('requirementsLabel')}</label>
              <textarea {...register('requirements')} rows={4} className={inputClass} placeholder={t('requirementsPlaceholder')} />
            </div>
            <div>
              <label className={labelClass}>{t('preferredLabel')}</label>
              <textarea {...register('preferred')} rows={4} className={inputClass} placeholder={t('preferredPlaceholder')} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <input {...register('requiredCompetencies')} className={inputClass} placeholder={t('competenciesPlaceholder')} />
            </div>
          </div>
        </div>

        <div className="flex items-center justify-end gap-3">
          <button type="button" onClick={() => router.back()} className="px-6 py-2.5 text-sm font-medium border border-border text-foreground hover:bg-background rounded-lg transition-colors duration-150">{t('cancelButton')}</button>
