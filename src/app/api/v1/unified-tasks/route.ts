  ...
  async (req: NextRequest, _context, user: SessionUser) => {
    ...
    const [leaveRaw, payrollRaw, onboardingRaw, offboardingRaw, performanceTasks, benefitRaw] = await Promise.all([
      ...
      (fetchLeave
        ...
        : Promise.resolve([])
      ).catch(err => { console.error('[unified-tasks] leave query failed:', err); return [] as never[] }),
      ...
      (fetchOffboarding
        ...
        : Promise.resolve([])
      ).catch(err => { console.error('[unified-tasks] offboarding query failed:', err); return [] as never[] }),
        ...
        if (delegatorIds.length > 0) {
          ...
          const delegatedTasks = leaveRequestMapper.toUnifiedTasks(
            ...
            metadata: {
              ...t.metadata,
              delegated: true,
              delegateeName: user.name,
            },
          }))

          // 중복 제거 (이미 leaveTasks에 있으면 제외)
          const existingIds = new Set(allTasks.map((t) => t.id))
          const newDelegated = delegatedTasks.filter((t) => !existingIds.has(t.id))
          allTasks = [...allTasks, ...newDelegated]
        }
      } catch (err) {
        console.error('[unified-tasks] delegation query failed:', err)
      }
    }
