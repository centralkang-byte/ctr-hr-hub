...
**source: `stage4-preflight/n48-wizardshell-ssot-shared.md` + batch 09 §N+48 (사용자 inquiry 결재 후)**:
> 결정 (Stage 3 Q1=A): proto WizardShell prop 시그니처 채택, Radix Dialog 기반
> pre-flight: ✅ `src/components/shared/` SSOT 정합 (15+ shared SSOT 패턴)

- **inquiry 결재 (Phase B 4/4 진입 사용자 round)**:
  - **Inquiry 1 (banner slot)** = 옵션 (c) **consumer-driven, banner prop 제외**. dead slot N+33 P2 패턴 재발 차단. codebase 측 `<DemoLimitBanner />` 실 컴포넌트는 별 RECORD = batch 04 §N+55 예약 (cross-batch consumer-driven 진입).
  - **Inquiry 2 (props 시그니처 SSOT 단일화)** = pre-flight 정합 surgical 정정. batch 09 §N+48 본문 8 props → **12 props** (banner 제외). 자세한 spec은 batch 09 §N+48 본문 참조.
- `src/components/shared/WizardShell.tsx` 신규 (~150 LOC) + vitest test (~60 LOC) + i18n 20 entries (선택, default footer 라벨 자체 보유 시)
- Radix Dialog 기반 + step indicator + footer override + **banner slot 미포함** (consumer-driven, N+49/N+55 동반)
...
| **N+48** | ~+150 (WizardShell, **12 props banner 제외**) + ~+60 (test) + ~+20 (i18n 선택) | 0 | **~+210 LOC** | 2-3 (`WizardShell.tsx` + `.test.tsx` + `messages/*.json`) | vitest + axe-core (Dialog focus trap) |
...
| **N+48 WizardShell** | ❌ 없음 — `src/components/shared/` 신규 codebase | ✅ N+21 (PR #64/#65) `DemoLimitBanner` ~~banner slot prop 통합~~ **consumer-driven (inquiry 1 옵션 c, banner prop 제외)** + proto `WizardShell` (`wizards.jsx`) 청사진 |
...
Phase A (proto SSOT layer, PR #64-#70 머지 후)
   ├─ N+21 (DemoLimitBanner) ──→ ~~N+48 banner slot 통합~~ **N+55 codebase 컴포넌트 신설 (consumer-driven, inquiry 1 옵션 c)**
...
| (proto) | N+21 DemoLimitBanner (PR #64) | ~~WizardShell `banner` slot prop 통합~~ **consumer-driven (inquiry 1 옵션 c)** — N+49 동반 또는 batch 04 §N+55 (codebase 측 실 컴포넌트) 별 RECORD |
