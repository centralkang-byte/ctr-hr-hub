...
> 의존성: 0 (독립 진입 가능)
> Stage 4 입력: `_design-reference/data.js` `ONBOARD_STEPS` 6단계 상수 신설 (~+30 LOC) + 4 카테고리 매핑 (DOCUMENT / TRAINING / INTRODUCTION / SETUP) schema `OnboardingTaskCategory` enum 정합

- **사전 가정 정정 5건** (`b6325ea6` audit → 실제 코드 cross-ref):
  1. 카테고리 enum 4종 **MEETING / ACCESS** → schema `OnboardingTaskCategory` 부재. **INTRODUCTION / SETUP** 매핑 정합 (schema enum 10종 안)
  2. "12 법인 idempotent" → schema `companyId String? // NULL = 글로벌 기본 템플릿` 의도. **global 1 + 법인 override 패턴** (단순 12 복제 X)
  3. "OnboardingTemplate default seed 신규" → `prisma/seed.ts:961-991` **`globalObTplId` upsert 이미 존재** (companyId=null, 7 task). 신규 신설 X
  4. "6단계 (proto 정합)" vs 기존 글로벌 7-task → **양면 비균질**. minimal 채택 = 기존 7 유지 + proto 6 별도 design 청사진 (운영 데이터 회귀 0)
  5. "proto `data.js` ONBOARD_STEPS SSOT 정합" → proto data.js 측에 `ONBOARD_STEPS` 상수 **부재** (`myTasks` 안 카테고리 hint만). 신설 필요
- **minimal scope (사용자 결재)**: `_design-reference/data.js` `ONBOARD_STEPS` 6단계 상수 신설 only (~+30 LOC, 1 file). prisma 측 변경 0 (기존 글로벌 7-task SSOT + CTR-KR/US 법인 override 유지).
...
> **Session 233 정정 cross-ref** (append-only): N+48 implementation 결과 inquiry round 결과 banner slot prop **부재** 채택 (옵션 c consumer-driven, PR #83 merge `90c88ac1`). actual `src/components/shared/WizardShell.tsx` 13 props (`title/sub/steps/currentStep/open/onCancel/onPrev/onNext/onSubmit/canProceed/footer/children/className`) — banner 처리는 consumer 측 children 안 inline render 패턴. cross-batch N+21 → N+48 banner slot 통합 가정 → N+55 (codebase `<DemoLimitBanner />` SSOT 신설, 별 PR 옵션 β) consumer wire 패턴 정정. 가디언 사전 가정 정정 acknowledge (catch 패턴 정합 — [09-wizardshell-ssot.md §7 N+48/N+49/N+55](../batch-cards/09-wizardshell-ssot.md) 본문 동반 정정).

> **Session 234 정정 cross-ref** (append-only): N+49 implementation 결과 inquiry round 결과 D1 결재 = default footer 채택 (옵션 α-3, PR #85 commit `144fcd66`). footer override partial 불가 catch (WizardShell.tsx L118 `footer ?? (default)` 전체 교체 패턴) → α-3 default 채택으로 SSOT 첫 consumer 검증 100% + wizard.* 5 i18n 키 전부 사용처 확보 (Codex Gate 2 dead code 위험 0). StickyActionBar 제거 부수효과 = mobile sticky-bottom UX 회귀 1점 → **N+56 schedule entry** (LOW shared, Phase 4 mobile polish 또는 mobile complaint 발생 시 진입). 가디언 사전 가정 정정 5건 acknowledge (props count 12→13 명목 / page.tsx +5 → 변경 0 / net LOC -75 → -84 / locale 5 정합 ko/en/zh/vi/es / newStepLabel 제거 5 entries) — [09-wizardshell-ssot.md §7 N+56](../batch-cards/09-wizardshell-ssot.md) schedule entry 동반 도착.
...
| **N+33** | ~+30 (proto data.js ONBOARD_STEPS) | 0 | **~+30 LOC** (minimal scope) | 1 (`_design-reference/data.js`) | 수동 cross-ref (schema enum 4 매핑) |
...
| **N+33** | `_design-reference/data.js` `ONBOARD_STEPS` 상수 (proto only, minimal) | proto data.js 기존 SSOT 패턴 정합 (N+19 PR #66 정합). **사전 가정 정정**: prisma 측 기존 글로벌 7-task `globalObTplId` upsert (`seed.ts:961-991`) 유지. minimal scope = proto 측만 신설. |
...
- N+33 seed: **사전 가정 정정 후 minimal scope** = `_design-reference/data.js` `ONBOARD_STEPS` 6단계 상수 신설 only. prisma 측 기존 글로벌 7-task `globalObTplId` 유지 (`seed.ts:961-991`, 회귀 0)
...
| **N+33** | MEDIUM | **LOW** (minimal 정정) | proto data.js `ONBOARD_STEPS` 1 file only, prisma 측 변경 0. 운영 데이터 회귀 0 | 기존 `globalObTplId` upsert 유지, proto 신규만 |
...
2. **N+33** (minimal 정정): proto data.js `ONBOARD_STEPS` 1 file only. prisma `globalObTplId` upsert (`seed.ts:961-991`) 미터치 → 운영 OnboardingTemplate 회귀 0
...
- **N+33 proto data SSOT** (minimal 정정): N+24 후, `_design-reference/data.js` `ONBOARD_STEPS` 6단계 상수 신설 only. prisma 변경 0 (기존 글로벌 7-task SSOT 유지)
