...
| Q2 | **링크 교정 + redirect 라우트 추가** | `workHourAlert.ts:214,226` + `useRecentPages.ts:68` → `/attendance` 일괄 갱신. **`/my/attendance` redirect 라우트 신설**(북마크 보호 + 휴가 패턴 일관성). ✅ 사전확인: `workHourAlert.ts`·`useRecentPages.ts` 둘 다 DO NOT TOUCH 부재 → 편집 가능 |
