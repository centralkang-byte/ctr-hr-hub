...
> 사용자 게이트 응답. Stage 4 작업계획·구현 입력. 휴가(#01 §7)와 묶음 진행.

| Q | 결정 | Stage 4 적용 |
|---|---|---|
| Q1 | **P0 확정** | 근태 = 실 운영 마일스톤 핵심 |
| Q2 | **링크 교정 = hotfix #58 분리 / redirect 라우트만 Stage 4 잔존** | 🔀 **링크 교정 부분은 hotfix [#58](https://github.com/centralkang-byte/ctr-hr-hub/pull/58)(`hotfix/work-hour-alert-link`, base main)로 분리·완료**: `workHourAlert.ts:214,226`→`/attendance`, `useRecentPages.ts:68` 스테일 엔트리 삭제(L41 `/attendance` 중복 회피). **Stage 4 잔존 = `/my/attendance` redirect 라우트 신설만**(북마크 보호 + 휴가 `/my/leave` 패턴 일관성). ✅ 사전확인: 두 파일 DO NOT TOUCH 부재. **Stage 4 작업계획 = hotfix #58 머지 후 진입**(G4 의존성) |
| Q3 | **동시 교정 OK + G1 강화** | G2 raw색·G3 EmptyState 동시. **G1 = toast만 불충분 → 낙관 UI 롤백 + 재시도 CTA + 에러사유 표시**까지. `AttendanceClient.tsx:145·154·207·224` 4지점 전부 |
| Q4 | **batch 포함** | **AT-004 = `status.ts STATUS_FG` home (chart.ts 부적합)** — 5 이산 상태(present→success/late→warning/absent→error/leave→info/overtime→accent), chart.ts HEATMAP_COLORS=연속 스펙트럼≠이산. N3확인: STATUS_FG 6카테고리 전수존재→status.ts 확장0, ATT_STATUS_TONE=컴포넌트-로컬. AT-005 = `WdGroupedStatCard` 2번째 수요(유지). chart.ts 2-shot §5 이연. (PR-2 audit 2026-05-19) |
| Q5 | **(B) `WdGroupedStatCard` 일반화 승격** | 휴가 §3.1 `WdLeaveBalanceCard` = 그 위 얇은 래퍼로 재구성(#01 백포팅 완료). 근태 AT-005 = 동일 베이스 별도 래퍼 |
| Q6 | **proto 7일 그리드 채택** | 코드 가로 바차트 폐기 → proto 7일 카드 그리드(1st principle). ⚠️ Stage 4 PR 코멘트 final check 노트: 가로 바=시간 비교 직관 우위 → 정보가치 손실 우려 사용자 최종 확인 |
| Q7 | **모바일 reflow M 동일** | 카드 reflow + 상단여백(batch 내), 사이드바 겹침=별도 트랙 |
| N | **#01 백포팅 완료** | 휴가 §3.1 = WdGroupedStatCard 베이스 + WdLeaveBalanceCard 래퍼 / §7 Q6 = (B-rewrite) 갱신. 본 단일 커밋에 동봉, 코드 0변경 |

**G4-followup (별도 보고 — Stage 4 진입 전)**: `/my/attendance` 팬텀 404 알림 =
**production-active 확정**. `checkWorkHourAlert`는 크론 아님 — `clock-out/route.ts:111`
에서 **퇴근 시마다 인라인 호출** → 48h 경고/52h 차단 시 `sendNotification` →
`prisma.notification.create`(notifications.ts:181) 실 in-app 레코드 생성, link
`/my/attendance`(라우트 부재) → 클릭 404. 주52시간 법정 알림이라 컴플라이언스
실사용 영향. **hotfix 트랙 분리 후보 — 사용자 즉시 판단 요청**(Stage 4 폴드 vs
독립 hotfix PR). 수정 자체는 2파일 문자열(trivial).
