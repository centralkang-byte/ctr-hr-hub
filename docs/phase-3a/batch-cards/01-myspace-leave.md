...
- 결정 SHA: 본 커밋
- **N+47 승격 (Session 236, PR #95)**: F14 정의 + 임계 5+ + `useArrowKeyNavigation` 가이드 +
  roving tabindex 표준 + 하이브리드 결정(신규 panel-less = radiogroup·aria-pressed / 기존
  tablist 2곳 N+9 grandfather 보존)이 `.claude/rules/accessibility.md` "## 탭·세그먼트·필터
  컨트롤" 섹션으로 승격 = a11y SSOT. N+43 훅 + N+44 적용 결과 명문화. axe-core baseline은
  인프라 트랙 후 별도. (audit "5 surface" → 실제 수동 2곳 drift 정정 동봉)
