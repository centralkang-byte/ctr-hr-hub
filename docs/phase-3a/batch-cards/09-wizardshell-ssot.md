...
| **N+51** | JobPostingWizard 마이그레이션 | MEDIUM | codebase | N+48 |
| **N+52** | PerfCycleWizard 마이그레이션 | MEDIUM | codebase | N+48 |
...
### N+51 — JobPostingWizard 마이그레이션 (Q4 점진 3) [MEDIUM]

- **트랙**: codebase
- **우선**: MEDIUM
- **의존성**: N+48 선행
- **Stage 4 입력**:
  - `src/app/(dashboard)/recruitment/new/PostingFormClient.tsx` 마이그레이션
  - Stage 4 pre-flight 시 정확한 inline 패턴 검증 + line delta 추정
  - i18n: caller 측 기존 키 재사용
- **Stage 4 검증**:
  - 기존 JobPosting 회귀 0
  - 시각 회귀 + e2e step 통과
...
### N+52 — PerfCycleWizard 마이그레이션 (Q4 점진 4) [MEDIUM]

- **트랙**: codebase
- **우선**: MEDIUM
- **의존성**: N+48 선행
- **Stage 4 입력**:
  - `src/app/(dashboard)/performance/PerformanceClient.tsx` (593) 또는 sub-page 마이그레이션
  - Stage 4 pre-flight 시 정확한 inline 패턴 검증
- **Stage 4 검증**: 기존 회귀 0
