...
> **inquiry 결재 (2건, 사용자 round)**:
> - **Inquiry 1 (banner slot 처리)** = 옵션 (c) **consumer-driven, banner prop 제외**. N+49 HireWorker 진입 시 동반 추가 (WizardShell 은 internal SSOT, evolve 자유). dead slot N+33 P2 패턴 재발 차단.
> - **Inquiry 2 (props 시그니처 SSOT 단일화)** = pre-flight 정합 surgical 정정. batch 09 §N+48 본 본문 spec 8 props → **12 props** (banner 제외, onPrev/onNext/onSubmit/canProceed/open 추가). default footer 동작 일관성 + 3 consumer footer override 강제 해소.
...
- **Stage 4 입력**:
  ...
  - **spec** (proto 정합 + inquiry 2 정정, 12 props):
    ```tsx
    ...
    interface WizardShellProps {
      ...
      open: boolean                    // Radix Dialog controlled
      onCancel: () => void
      onPrev?: () => void              // default footer 사용 시 필수
      onNext?: () => void              // default footer 사용 시 필수
      onSubmit?: () => void            // 마지막 step
      canProceed?: boolean             // default true; false 면 next/submit disabled
