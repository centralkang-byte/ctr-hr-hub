...
## 조치 #4 — Bucket D #2 퇴사 완료 Assignment.endDate 마감 (PR #114, S254+)

Bucket D #2 해소. `executeOffboardingCompletion`이 `offboarding.status=COMPLETED`만 set하고 직원 primary assignment endDate를 닫지 않아, 퇴사자가 `endDate:null` 활성 레코드 쿼리(CLAUDE.md SSOT 컨벤션)에 계속 잡히던 데이터정합 버그. `complete-offboarding.ts` tx step 5b에 `tx.employeeAssignment.updateMany`로 `{ employeeId, isPrimary:true, endDate:null }`에 `endDate=lastWorkingDate` set (start route의 status=RESIGNED/TERMINATED where와 동일 → 정합). 런타임 dogfood(HR, 공유 DB): 퇴사 완료 → 완료 200·정산 4건 → primary assignment end_date=lastWorkingDate 마감(DB 직접 조회), 직원이 active 목록/상세(404)에서 정상 제외, 완료 전엔 end_date=null. tsc·eslint PASS. → **[PR #114](https://github.com/centralkang-byte/ctr-hr-hub/pull/114)**.

## 조치 #5 — Bucket D #5 GET /onboarding/instances 회사 스코프 강제 (PR #121, S256)

Bucket D #5 해소. `GET /api/v1/onboarding/instances`가 `_user`를 무시하고 `companyId` 쿼리 파라미터를 그대로 where에 사용 → 파라미터 없으면 전 법인 노출, 비-SUPER HR_ADMIN이 타 법인 id 주입으로 타 법인 온보딩 조회 가능(#119 cross-company write와 동일 클래스). fail-closed 가드: 비-SUPER는 `user.companyId` 강제, SUPER만 파라미터로 타 법인/전체 조회. **컬럼 기반 스코프**(`EmployeeOnboarding.companyId` — 생성 4경로 모두 세팅) 채택 — assignments 기반(형제 offboarding 패턴)은 빈-assignment 인스턴스를 숨김. resolveCompanyId는 SUPER+무param을 user.companyId로 제한해 전 법인 뷰를 회귀시키므로 list 엔드포인트엔 부적합. tsc·eslint·Codex Gate 2(0) + 신규 e2e(companyId 파라미터 무력화 가드)·런타임 curl 검증. → **[PR #121](https://github.com/centralkang-byte/ctr-hr-hub/pull/121)**.

## 조치 #6 — Bucket D #6 (오탐 기각) + #7 퇴사 조회 모듈 라벨 교정 (S256+)

런타임 실관측(dev :3002, 실 세션, **수정 전/후 직접 비교**)으로 트리아지 #6·#7 전제를 재검증 → 둘 다 감사 과보고였음을 확인([[phase3a-audit-drift]], [[p0-audit-overreports-wiring]]):

- **#6 휴가 admin page role 가드 = 오탐 기각 (코드 변경 없음).** `/leave/admin`은 이미 미들웨어 RBAC가 차단 — `src/lib/rbac/rbac-spec.ts:49` `{ prefix: '/leave/admin', allowedRoles: HR_UP }` (HR_UP = HR_ADMIN·SUPER_ADMIN). **수정 전(인라인 가드 없는 origin/main 원본)에도** manager·employee·executive → `307 /?error=forbidden` (`src/middleware.ts:226`). 감사 주장 "MANAGER/EXEC가 깨진 빈 페이지 진입"은 **재현 불가**(페이지 도달 전 미들웨어 차단; HR_UP만 통과해 정상 200). 컨벤션 확정: rbac-spec 커버 admin 페이지(`payroll`·`compliance`·`attendance/admin`)는 **전부 인라인 가드 없이 미들웨어 의존** → 인라인 가드 추가는 **중복 dead code + 컨벤션 위반 outlier**. 감사가 "page 인라인 가드 없음"만 보고 미들웨어 커버리지를 놓침(STATUS의 "#6 재평가 REAL"도 동일 드리프트). `hr/bulk-movements`가 인라인 가드를 갖는 건 rbac-spec에 **없는** 예외라서(=미들웨어 미커버 라우트에만 인라인 가드).
- **#7 퇴사 조회 모듈 라벨 교정 (코드 1줄, 유지).** `employees/[id]/offboarding/route.ts:54` GET이 offboarding 데이터를 `perm(MODULE.ONBOARDING, VIEW)`로 가드 → `MODULE.OFFBOARDING`로 교정. 형제 `offboarding/start/route.ts:296`이 이미 OFFBOARDING 사용 = 정답 선례. **런타임 검증: 수정 전/후 5개 role 응답 완전 동일**(hr/super=404 perm통과·exec/mgr/emp=403) — 현 권한 매트릭스는 HR/SUPER가 onboarding·offboarding view 둘 다 보유, 나머지 둘 다 미보유 → **현재 동작 변화 0**. 즉 live 버그 아닌 **잠재 correctness/하드닝**(onboarding-only 역할 신설 시 퇴사 데이터 누출 예방). employees API는 rbac-spec 미커버라 route `withPermission`이 유일 가드. tsc·eslint PASS.

순효과 = 코드 1파일(#7) + 본 doc. #6은 코드 미변경(오탐 기각). #8~#10은 미착수. → **[PR #122](https://github.com/centralkang-byte/ctr-hr-hub/pull/122)**.
