...
- **[정정 2] 카나리 일시 깨짐**: PR-1 머지 시 휴가 `/leave`는 LV-001만 신규
  컴포넌트 → 페이지 시각 일관성 일시 깨짐(PR-3서 해소). PR-1↔PR-3 간격 짧게
  (3일 이내/동일 sprint), PR-1 description에 명시.

### WS-B. `chart.ts` 1-shot (LV-002) + `status.ts` (AT-004) — 전제 정정

> **[정정 — PR-2 사전 audit 발견, 가디언 승인 2026-05-19]**: 당초 전제
> "chart.ts 2-shot = LV-002+AT-004" 는 **오류**. audit 결과:
> - **LV-002**(휴가 월별 사용 막대, 단일 시리즈) = `chart.ts` **정확 수용**
>   (`CHART_THEME.colors[0]` navy wt + axis/grid/tooltip). chart.ts 설계 목적
>   그대로 → **1-shot clean** 신규 소비처. 확장 0.
> - **AT-004**(근태 30일, present/late/absent/leave/overtime **5 이산 상태**)
>   = `chart.ts` **부적합** (HEATMAP_COLORS=연속 good→bad 스펙트럼 ≠ 5 이산
>   상태, 카테고리 시리즈도 아님). 진짜 SSOT = **`status.ts STATUS_FG`**
>   (이산 상태 의미색). 매핑 present→success/late→warning/absent→error/
>   leave→info/overtime→accent. **N3 확인: STATUS_FG 6 카테고리 전수 존재
>   → status.ts 확장 0**, `ATT_STATUS_TONE`(AttStatusKey→StatusCategory)는
>   컴포넌트-로컬 상수(SSOT 무변경).
- 결과: **chart.ts 2-shot 압박 미발생 → escape hatch(chart.ts 확장 서브트랙)
  미트리거.** chart.ts = LV-002 1개 신규 소비처만 clean 검증.
- **chart.ts 2-shot 검증 = 다음 batch 로 이연**(조직변경·평가·리뷰 사이클 등
  신규 chart 소비처 출현 시 시도). §5 회고 등록.
- 순서: WS-A 후, 페이지 적용(WS-F) 전 LV-002 chart 소비 형태 + AT-004
  status.ts 매핑 확정.
...
| PR-2 | WS-B chart.ts **1-shot LV-002** + **status.ts AT-004** 형태 확정 (2-shot 이연) | #59 머지 SHA | PR-1(#59) |
| PR-3 | WS-F 휴가 `/leave` reskin + WS-D 휴가 가디언 + WS-C 휴가 redirect **[조건] description 명세 의무** | PR-2 | PR-1,2 |
| PR-4 | WS-F 근태 `/attendance` reskin + WS-D 근태 가디언(G1강화) + WS-C 근태 redirect | PR-3 | PR-3 |
| PR-5 | WS-E i18n 라벨 정합 | PR-4 | PR-4 |

> 단일 거대 PR 회피(블라스트 분리). PR-2 = chart.ts 1-shot(LV-002) clean +
> status.ts(AT-004) home. chart.ts 2-shot 압박 미발생(escape hatch 미트리거).

> **[정정 1 — PR-3 surface 큼, 유지(분리비용>합침비용) + description 명세 의무]**:
> PR-3 description에 ① **변경 surface = 파일·라인 list** ② **e2e 시나리오 5개+**
> 필수 명세 — (1) 신청→PENDING→취소 (2) redirect 딥링크 6곳 (3) 잔여 카드
> 홈/허브 인라인 동작 (4) 가디언 위반 회귀(LV-005 칩·L542 색·무음catch·EmptyState)
> (5) 모바일 reflow(`.tbl-as-cards`). 명세 누락 시 PR-3 리뷰 진입 불가.

> **[정정 2 — PR-1 카나리 페이지 일관성 일시 깨짐]**: PR-1 머지 시 휴가 `/leave`는
> LV-001만 새 `WdGroupedStatCard`로 전환되어 페이지 내 **시각 일관성 일시 깨짐**
> (나머지 섹션 구 스타일). PR-3 머지 시 해소. **PR-1↔PR-3 머지 간격 짧게 권장
> (3일 이내 또는 동일 sprint)**. 동일 노트를 PR-1 description에도 명시.
...
- **[정정 3 v2 — PR-1·PR-2 e2e PASS 의 정확한 의미 (D4 갱신)]**:
  - **CI**: 가드 동작 검증만 — prod NODE_ENV(`npm run start`) 시 `/dev/*` 404.
  - **로컬 dev**: 컴포넌트 시각 픽스처(cards/rows/tone/empty) — `/dev/components`.
  - **CI 에서 컴포넌트 회귀 자동 detect 불가** — 로컬 의존(spec `process.env.CI` 분기).
  - 페이지 회귀는 PR-3 이후 e2e PASS 로 누적 검증.
  - **SSOT 컴포넌트 5+ 누적 시 인프라 재평가 트랙 등록** (Playwright CT 또는
    visual regression 도구 도입 후보).
...
- **🔭 회고 이연 (Phase 3a 종료 회고 등록)**: `chart.ts` **2-shot SSOT 검증
  = 다음 batch 로 이연**. 본 batch 는 chart.ts 1-shot(LV-002)만 — 2번째
  이종 소비처 부재(AT-004는 status.ts home). 조직변경·평가·리뷰 사이클 등
  신규 chart 소비처 출현 batch 에서 2-shot 시도 → chart.ts API 일반화
  안정성 그때 판정. (N5)
...
| 3 | ~~chart.ts 2-shot~~ → **chart.ts 1-shot(LV-002) + status.ts(AT-004)** | §2 WS-B 정정 (audit: AT-004 chart.ts 부적합→status.ts home, 2-shot §5 이연) |
