...
## REVIEW-1 완료 (2026-03-03)

> 범위: B1~B10 전체 (1,233 TS 파일, 290+ API 라우트, 182 Prisma 모델)
> 보고서: `REVIEW_1_REPORT.md` (프로젝트 루트)

### 발견 사항

- 🔴 Critical 4건: payroll/anomalies companyId 누락, payroll/import-logs IDOR, vector-search Prisma.raw() SQL injection, analytics/calculate DB connection pool 고갈
- 🟠 Major 6건: employee-risk 법인 간 IDOR, teams/config webhook URL 평문 감사로그, N+1 쿼리 14파일, 무한 findMany 40+라우트, soft delete 4패턴 혼재, 응답형식 불일치 16파일
- 🟡 Minor 5건: DB 인덱스 누락(9모델), as any 78개, process.env 직접접근 2개, String status enum 미변환 11개, @db.Uuid 미적용
- 💡 Suggestion 3건: Json 정규화 후보, Employee 단방향 관계 명시, INTERVAL 직접삽입

### 잘 된 점

tsc 0 errors ✅ | XSS 없음 ✅ | 하드코딩 시크릿 없음 ✅ | Cron/Webhook 인증 ✅ | withPermission 290+ 라우트 ✅ | Prisma 네이밍 182모델 일관 ✅

### 다음 단계

B11 착수 전 🔴 Critical 4건 + 🟠 Major 중 M-1(IDOR), M-2(webhook URL) 수정 필수.
나머지 Major/Minor는 FIX-1 세션에서 일괄 처리.
