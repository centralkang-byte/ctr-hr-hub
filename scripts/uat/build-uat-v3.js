// CTR HR Hub UAT Guide v3.0 generator — Wave 1 IA 재편(2026-06) + S338 UAT 워크스루 반영
//
// Run: cd scripts/uat && npm install && node build-uat-v3.js
// Output: docs/uat/UAT_가이드_v3.docx
// Screenshots: docs/uat/screenshots-v3/ (scripts/capture-uat-v3.ts 로 재캡처)
//
// Source of truth for the UAT guide v3. Modify text/structure here and regenerate.
...
const SHOTS = path.join(REPO_ROOT, 'docs/uat/screenshots-v3');
const OUT = path.join(REPO_ROOT, 'docs/uat/UAT_가이드_v3.docx');

// ---------- Configurable constants ----------
const STAGING_URL = 'https://ctr-hr-hub-git-staging-sangwoos-projects-b01c065c.vercel.app';
const CAPTURE_COMMIT = '0f5f00b3';
const CAPTURE_DATE = '2026-07-12';
  ...
  return [
    ...
    buildTable(
      ...
      [
        ...
        ['대상', '인사 담당자 (UAT 테스터)'],
        ['분류', '내부 문서 — 대외비'],
        ['버전 이력', 'v2 (5/11) → v3: 메뉴 개편(Wave 1) 반영, 화면 재캡처, 알려진 이슈 갱신'],
      ],
      [3000, 6360],
    ),
    pageBreak(),
...
function section5() {
  return [
    ...
    pageBreak(),
    h2('5.2 나의 업무 · 승인 요청 (구 결재함)'),
    p('결재 처리는 별도 결재함 메뉴가 아니라, 왼쪽 메뉴의 "나의 공간 > 나의 업무" 화면에 있는 "승인 요청" 탭에서 합니다. 팀장/인사담당자/전사관리자에게만 승인 요청 탭이 보이며, 휴가·성과·급여 결재 대기 건을 한 화면에서 승인/반려할 수 있습니다.'),
    p('직원에게는 승인 요청 탭이 표시되지 않습니다. 본인이 올린 신청의 진행 상태는 각 모듈 화면(예: 휴가/휴직의 신청 내역)과 알림에서 확인합니다.'),

    h3('5.2.1 직원 — 나의 업무 (내 할 일)'),
    ...image('approvals-inbox-employee.png', '그림 5-5. 직원 역할 나의 업무 (승인 요청 탭 없음)'),

    h3('5.2.2 팀장 — 승인 요청 탭'),
    ...image('approvals-inbox-manager.png', '그림 5-6. 팀장 역할 승인 요청 탭'),

    h3('5.2.3 인사담당자 — 승인 요청 탭'),
    ...image('approvals-inbox-hr-admin.png', '그림 5-7. 인사담당자 역할 승인 요청 탭'),

    h3('5.2.4 전사관리자 — 승인 요청 탭'),
    ...image('approvals-inbox-super-admin.png', '그림 5-8. 전사관리자 역할 승인 요청 탭'),
...
function section6() {
  return [
    ...
    p('※ 나머지 특수 상황 직원은 테스트북(UAT_테스트북_v3.xlsx)의 엣지케이스 시트를 참고해 주세요.'),
    new Paragraph({
      spacing: { before: 120, after: 120 },
      indent: { left: 240, right: 240 },
      shading: { fill: 'FFF4E5', type: ShadingType.CLEAR, color: 'auto' },
      children: [
        new TextRun({ text: '⚠ 시간 의존 데이터 주의: ', font: FONT, size: 22, bold: true, color: 'B85C00' }),
        new TextRun({
          text: '수습 만료일·계약 만료일·지각/초과근무 기록 등 날짜가 박힌 시드 데이터는 시간이 지나며 낡습니다(예: 지각왕·과로자의 기록이 최근 30일 밖). 해당 항목은 테스트북 비고에 "시드 노화"로 표시되어 있으며, 데이터 초기화 후 재검합니다.',
          font: FONT,
          size: 22,
        }),
      ],
    ...
    buildTable(
      ...
      [
        ...
        ['연차 잔액', '이민준, 정다은 — 초기화 직후 각 15일 (테스트 진행에 따라 변동 — 테스트북 휴가관리 Step 1에서 현재 잔액 확인 후 진행)'],
      ],
      [3000, 6360],
    ),
  ];
...
function section8() {
  return [
    h1('8. 이슈 리포트 규칙'),
    p('테스트 시나리오는 UAT_테스트북_v3.xlsx 를 사용합니다. 이슈가 발견되면 별도 피드백 Excel 파일에 아래 정보를 기록해 주세요. 재현 절차가 정확할수록 수정이 빨라집니다.'),
    buildTable(
      ['#', '항목', '작성 방법', '예시'],
      [
...
function section9() {
  return [
    ...
    buildTable(
      ['#', '내용', '상세 설명'],
      [
        ['1', '영문 이름 없는 직원 정보 수정 오류', '영문 이름 없이 등록된 직원의 정보를 수정하면 "잘못된 요청 데이터입니다" 오류가 납니다. 수정 예정.'],
        ['2', '직원 계약 이력 화면 문제', '직원 상세에 계약 이력으로 가는 버튼이 없고, 계약이 없는 직원은 화면이 "처리 중"에서 멈춥니다. 수정 예정.'],
        ['3', '수습/계약 만료 상태 화면 미표시', '수습 중·계약 만료 직원이 화면에서 구분되지 않고 만료 경고도 없습니다. 수정 예정.'],
        ['4', '휴가 신청 창의 잔액 미리보기 오표시', '휴가 신청 창에 뜨는 잔여 일수 숫자가 실제와 다를 수 있습니다. 실제 잔액은 휴가 화면의 잔액 카드를 기준으로 확인해 주세요. 수정 예정.'],
        ['5', '휴무일 출근이 "지각"으로 표기', '주말·공휴일에 출근하면 지각으로 집계될 수 있습니다. 수정 예정.'],
        ['6', '휴직/퇴직자가 근태 목록에 포함', '휴직 중이거나 퇴직한 직원이 근태 관리 목록에 "미출근"으로 집계될 수 있습니다. 수정 예정.'],
        ['7', '직원 출퇴근 수정 요청 기능 없음', '직원이 본인 출퇴근 기록의 수정을 요청하는 기능은 현재 없습니다(수정은 인사담당자가 처리). 도입 여부 검토 중 — 버그로 보고하지 않으셔도 됩니다.'],
        ['8', '직원 등록 확인 단계 "데모 화면" 문구', '신규 직원 등록 마지막 단계에 데모 안내 문구가 남아 있습니다. 실제로는 정상 저장됩니다. 수정 예정.'],
        ['9', '비정기 보상 변경 알림 미발송', '정기 보상 외 임시 보상 조정 시 결재 알림이 자동 발송되지 않습니다.'],
        ['10', '발령일 날짜 계산 1일 오차 가능', '시간대 처리 차이로 인해 발령일이 1일 다르게 보일 수 있습니다.'],
        ['11', '법인 코드 변경 시 캐시 새로고침 필요', '법인 코드 변경 직후 화면이 즉시 반영되지 않을 수 있으며, 잠시 후 새로고침 시 정상 표시됩니다.'],
      ],
      [500, 2860, 6000],
    ),
  ];
