...
## 13. 기능 충실도 (연결 / 구현 / 부분구현)

시그니처 컴포넌트를 실페이지에 카나리 적용할 때, 컴포넌트가 노출하는
**액션은 시각만이 아니라 기능까지 충실해야** 한다. mock·stub·"준비 중
disabled" 로 시각만 채우는 것 금지 (시그니처 외 보조 액션 한정 예외).

- **(가) 완전 존재** — 기존 기능이 end-to-end 작동 → 시그니처 컴포넌트와
  **연결만**. 동작·권한·i18n 보존, 신규 구현 0.
- **(나) 부분 존재** — 일부 레이어만 존재(API만 / UI만 / 토스트 없음 등)
  → **누락 레이어만 구현** + 연결.
- **(다) 미존재** — 어느 레이어도 없음 → **전체 구현** (권한·API·DB·
  상태·UX 끝까지).

판정 7레이어: Prisma mutation / API(route+validation+RLS) / 권한 가드 /
FE mutation / UI 트리거 / 사용자 피드백 / 상태 갱신. 사전 audit 를 액션 ×
7레이어 매트릭스 표로 보고. 누락 구현이 시그니처 범위를 크게 초과하면
사전 보고 후 사용자 판단(포함 vs 별도 트랙). 자세한 절차·게이트는
프로젝트 `CLAUDE.md` "카나리 작업 표준" 참조.

### 13-1. 색 토큰 통합 변형 (시그니처 아님)

도메인 색을 SSOT 팔레트(예 Workday `--wt-*`)로 통합하는 트랙은 위 7레이어
대신 **색 매핑 표**(토큰별 신값/기존매핑/영향 셀렉터)로 audit하고,
**시각 회귀 3축**(변경 셀렉터 computed-style 신값 / 나머지 토큰 회귀 0 /
chrome PNG)으로 검증. 색공간 변환은 **수동·근사 금지**(브라우저 엔진/검증
라이브러리). 신값은 적용 전 swatch 사전 보고. 다소비처 토큰(차트 10+)은
저블라스트와 별 서브트랙 분리. 절차는 `CLAUDE.md` "P2 토큰통합 트랙 변형".
